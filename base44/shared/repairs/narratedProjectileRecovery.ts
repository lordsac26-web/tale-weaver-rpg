...
const objectEntries = (value, path='session', out=[], searched=[]) => { if (Array.isArray(value)) { searched.push(path); value.forEach((entry,index)=>objectEntries(entry,`${path}[${index}]`,out,searched)); } else if (value && typeof value === 'object') { const action=value.player_choice ?? value.action_text ?? value.action ?? value.choice ?? value.input; const narration=value.text ?? value.narrative ?? value.result ?? value.response ?? value.description; if (typeof action==='string' || typeof narration==='string') out.push({path,value,action:String(action || ''),narration:String(narration || '')}); searched.push(path); Object.entries(value).forEach(([key,entry])=>objectEntries(entry,`${path}.${key}`,out,searched)); } return {out,searched}; };

export async function discoverNarratedProjectileRecovery({ db, characterId, sessionId }) {
  const [character,session,combats]=await Promise.all([db.entities.Character.get(characterId).catch(()=>null),db.entities.GameSession.get(sessionId).catch(()=>null),db.entities.CombatLog.filter({session_id:sessionId},'-created_date',20)]);
  ...
  const surfaces=objectEntries(session); const completed=(combats||[]).filter((combat)=>!combat?.is_active&&['victory','defeat','fled','resolved'].includes(combat?.result));
  const candidates=surfaces.out.map((node)=>{const match=node.narration.match(/\b(?:found|recover(?:ed)?|retrieve[ds]?|collect(?:ed)?)\b[^.]{0,120}\b(7)\s+arrows\b/i);const check=node.value.skill_check||node.value.authoritative_skill_check||node.value.check||node.value.result?.check||null;const actionOK=/\blook around\b.*\b(?:arrow|arrows)\b|\b(?:search|look|scan)\b[^.]{0,80}\b(?:arrow|arrows)\b/i.test(node.action);const checkOK=check?.success===true||/\bperception\b[^.]{0,100}\bsuccess\b/i.test(`${node.action} ${node.narration}`);return match&&actionOK&&checkOK?{...node,quantity:7,check,content_hash:null}:null;}).filter(Boolean);
  const candidate=candidates.length===1?candidates[0]:null; if(candidate) candidate.content_hash=await hash(candidate.value);
  const later=candidate?surfaces.out.filter((node)=>node.path>candidate.path&&/\b(?:arrow|arrows|recover|retrieve|collect)\b/i.test(`${node.action} ${node.narration}`)).map((node)=>({path:node.path,timestamp:node.value.timestamp||null,action:node.action.slice(0,160)})):[];
  const protectedHashes={character:await hash(character),session:await hash(session),inventory:await hash(character.inventory||[]),completed_combats:await hash(completed.map(semanticCombat))}; const ambiguity=[];if(candidates.length!==1)ambiguity.push(`expected_one_candidate_found_${candidates.length}`);if(later.length)ambiguity.push('later_recovery_conflict');
  const incident_key=candidate?await hash({session_id:sessionId,character_id:characterId,path:candidate.path,timestamp:candidate.value.timestamp||null,action:normalizedText(candidate.action),narration:normalizedText(candidate.narration),item:'Arrows',quantity:7}):null;
  const discovery={searched_fields:[...new Set(surfaces.searched)].filter((path)=>/story|narrative|history|scene|result|action/i.test(path)),searched_object_count:surfaces.out.length,match_count:candidates.length,candidate_story_path:candidate?.path||null,candidate_story_index:(candidate?.path.match(/\[(\d+)\]$/)||[])[1]??null,candidate_timestamp:candidate?.value.timestamp||null,player_action:candidate?.action||null,narrated_text_excerpt:candidate?.narration.slice(0,280)||null,skill_check_result:candidate?.check||null,recovered_item:candidate?{canonical_item:'Arrows',quantity:7}:null,incident_key,content_hash:candidate?.content_hash||null,current_inventory_hash:protectedHashes.inventory,session_hash:protectedHashes.session,current_canonical_stack_quantity:quantity(character.inventory,'Arrows'),expected_post_repair_quantity:candidate?quantity(character.inventory,'Arrows')+7:null,later_conflict_scan:later,ambiguity_diagnostics:ambiguity,completed_combat_context:completed.map((entry)=>({id:entry.id,result:entry.result,created_date:entry.created_date})),protected_hashes:protectedHashes};
  return ambiguity.length?{status:409,body:{success:false,mode:'discover',writes:0,discovery}}:{status:200,body:{success:true,mode:'discover',writes:0,discovery,protected_hashes:protectedHashes}};
}

export async function reconcileLegacyNarratedProjectileRecovery({db,characterId,sessionId,incidentKey,expectedHashes,mode}) { const discovered=await discoverNarratedProjectileRecovery({db,characterId,sessionId}); const d=discovered.body.discovery; if(!d||discovered.status!==200||d.incident_key!==incidentKey||!expectedHashes||Object.entries(d.protected_hashes).some(([key,value])=>expectedHashes[key]!==value))return {status:409,body:{error:'Legacy incident guard failed',writes:0,discovery:d}}; if(mode==='dry_run')return {status:200,body:{success:true,mode,writes:0,discovery:d}}; const committed=await executeRecoveryTransaction({base44:{asServiceRole:db},sessionId,characterId,requestId:incidentKey,outcome:{check:{success:true},recovery:{type:'arrows',quantity:7}}}); return {status:committed.status,body:{success:committed.body?.applied===true,mode,writes:committed.body?.writes||0,already_processed:committed.body?.already_processed||false,receipt:committed.body?.receipt||null,discovery:d,error:committed.body?.reason}}; }