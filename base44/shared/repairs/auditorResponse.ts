...
export function formatAuditorResponse(body,responseFormat){
  ...
  const compact={success:body?.success===true,mode:body?.mode||null,function_version:body?.function_version||null,classification:body?.classification||null,safe_to_repair:body?.safe_to_repair===true,writes:Number(body?.writes||0),all_guards_true:failedGuards.length===0,...(Object.hasOwn(body||{},'arrows_to_deduct')?{arrows_to_deduct:body.arrows_to_deduct,quantity_before:body.quantity_before,quantity_after:body.quantity_after}:{}),proposal_hash:body?.proposal_hash||null,apply_token:body?.apply_token||null};
