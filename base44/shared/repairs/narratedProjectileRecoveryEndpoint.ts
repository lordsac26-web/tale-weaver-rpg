...
import { auditRepairNarratedProjectileRecovery, discoverNarratedProjectileRecovery, reconcileLegacyNarratedProjectileRecovery, NARRATED_PROJECTILE_RECOVERY_CONTRACT } from './narratedProjectileRecovery.ts';
...
export async function handleNarratedProjectileRecoveryAudit({ base44, rawBody }) {
  ...
  if (body.incident_key && ['dry_run','apply'].includes(body.mode) && body.character_id && body.session_id) {
    const result=await reconcileLegacyNarratedProjectileRecovery({db:base44.asServiceRole,characterId:body.character_id,sessionId:body.session_id,incidentKey:body.incident_key,expectedHashes:body.expected_hashes,mode:body.mode}); return Response.json(body.compact ? compact(result.body) : result.body,{status:result.status});
  }
