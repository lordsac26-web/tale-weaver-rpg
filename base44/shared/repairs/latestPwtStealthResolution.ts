...
export async function auditRepairLatestPwtStealth({ db, scope, requestId, mode = 'dry_run', preconditionHashes = null }) {
  ...
  const minimumTotal = expectedBreakdown.ok ? 1 + expectedBreakdown.total : null;
  const deterministicSuccessWithoutD20 = !Number.isFinite(raw) && Number.isFinite(minimumTotal) && minimumTotal >= dc;
  const expectedResult = deterministicSuccessWithoutD20 ? true : Number.isFinite(raw) && expectedBreakdown.ok ? (raw !== 1 && (raw === 20 || expectedTotal >= dc)) : null;
  ...
  const guards = { exact_linkage: session.character_id === scope.characterId, unique_latest_action: candidates.length === 1 && target?.index === LATEST_PWT_STEALTH_CONTRACT.storyIndex, authoritative_modifier_unambiguous: expectedBreakdown.ok === true, no_attack_or_damage_mutation: attackEvidence.length === 0, original_d20_available: Number.isFinite(raw), deterministic_success_without_d20: Number.isFinite(raw) || deterministicSuccessWithoutD20, unique_receipt: receiptCandidates.length <= 1, exact_precondition_hashes: hashesMatch };
  const failedGuards = Object.entries(guards).filter(([name, pass]) => !pass && name !== 'original_d20_available').map(([name]) => name);
  const legacyNote = deterministicSuccessWithoutD20 ? `Original d20 was not persisted; minimum legal total ${minimumTotal} (${expectedBreakdown.base_skill} base + ${expectedBreakdown.effect_bonus} effects + minimum d20 1) proves SUCCESS against DC${dc}.` : null;
  const diagnostics = { success: failedGuards.length === 0, dry_run: mode === 'dry_run', request_id: requestId, mode, writes: 0, guards, failed_guards: failedGuards, latest_action: { story_index: target?.index ?? null, timestamp: target?.entry?.timestamp || null, request_id: target?.entry?.request_id || null, player_choice: target?.entry?.player_choice || null, narrative: target?.entry?.text || null }, recorded: { d20: raw, dc, modifier: recordedModifier, total: recordedTotal, result: recordedResult, receipt: receipt || null }, expected: { modifier: expectedBreakdown.total, total: expectedTotal, minimum_total: minimumTotal, result: expectedResult, breakdown: expectedBreakdown, legacy_note: legacyNote }, derived_state: { stealthed_count: stealthed.length, stealthed }, attack_or_damage_already_occurred: attackEvidence.length > 0, attack_evidence: attackEvidence, protected_hashes: protectedHashes };
  ...
  const correctedReceipt = deterministicSuccessWithoutD20 ? { id: target.entry.request_id || requestId, request_id: target.entry.request_id || requestId, skill: expectedBreakdown.skill, raw_d20: null, all_rolls: [], dc, modifier_total: expectedBreakdown.total, final_total: null, minimum_total: minimumTotal, success: true, modifier_breakdown: expectedBreakdown, advantage_sources: receipt?.advantage_sources || [], at: target.entry.timestamp, legacy_note: legacyNote } : buildSkillCheckReceipt({ requestId: target.entry.request_id || requestId, raw, allRolls: receipt?.all_rolls || receipt?.allRolls || [raw], dc, success: expectedResult, breakdown: expectedBreakdown, advantageSources: receipt?.advantage_sources || [] });
  const attribution = deterministicSuccessWithoutD20 ? `Stealth DC${dc} — SUCCESS (minimum total ${minimumTotal}; base ${expectedBreakdown.base_skill} + Pass without Trace 10 + minimum d20 1; original d20 not persisted)` : `Stealth DC${dc} — ${expectedResult ? 'SUCCESS' : 'FAILURE'} (d20 ${raw} + base ${expectedBreakdown.base_skill} + Pass without Trace 10 = ${expectedTotal})`;
  ...
  return { status: 200, body: { success: true, already_processed: false, request_id: requestId, writes: JSON.stringify(nextConditions) === JSON.stringify(character.conditions || []) ? 1 : 2, original_d20_reused: raw, deterministic_success_without_d20: deterministicSuccessWithoutD20, minimum_total: minimumTotal, outcome_changed: currentOutcome !== expectedResult, corrected_receipt: correctedReceipt, attribution, protected_hashes: protectedHashes } };
}