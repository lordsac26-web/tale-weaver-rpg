...
export async function handlePlayerAttack(ctx) {
  ...
  const rollBreakdown = { roll_type: 'attack', roll_origin: payload?.roll_submission?.origin === 'player' ? 'player' : 'ai', dice: { mode: attackResult.advantage ? 'advantage' : attackResult.disadvantage ? 'disadvantage' : 'normal', rolls: attackResult.rolls, selected: attackResult.roll, advantage_sources: advantageSources, disadvantage_sources: disadvantageSources }, modifiers: attackModifierComponents, modifier_total: attackMod, final_total: totalAttack, target_ac: target.ac + targetACBonus };
