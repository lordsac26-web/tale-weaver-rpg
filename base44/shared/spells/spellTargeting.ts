...
export const SPELL_TARGETING_VERSION = 'canonical-spell-targeting-v1.0.1';
...
export function canonicalSpellTargetProfile(spell:any={}) {
  ...
  return {version:SPELL_TARGETING_VERSION,kind:pointTarget?'point_area':creatureTarget?'creature':'unspecified',range_feet:range,area_radius_feet:radius,concentration:spell.concentration===true,casting_time:String(spell.casting_time||''),components:String(spell.components||''),duration:String(spell.duration||''),canonical_name:String(spell.name||''),canonical_id:String(spell.id||spell.raw_data?.slug||spell.raw_data?.key||'')||null};
}
...
export function resolveCanonicalSpellTarget({spell,actionText,target}:any) {
  ...
  if(profile.kind==='point_area'){
    ...
    const supplied=target&&typeof target==='object'&&['point','point_area'].includes(target.kind)?String(target.anchor||'').trim():'';
    ...
    return {ok:true,status:200,profile,target:{kind:'point_area',anchor:normalizeSpellText(anchor),ref:target?.ref||null,coordinates:target?.coordinates||null,display_anchor:anchor,distance_feet:Number.isFinite(distance)?distance:null,within_range:!Number.isFinite(distance)||!profile.range_feet||distance<=profile.range_feet,stationary:true,radius_feet:profile.area_radius_feet,range_feet:profile.range_feet}};
  }
