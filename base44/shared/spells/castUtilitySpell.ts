...
const durationGameHours = (spell) => { const match=String(spell?.duration||'').match(/(\d+(?:\.\d+)?)\s*(minute|hour|round)/i); if(!match)return null; const amount=Number(match[1]),unit=match[2].toLowerCase(); return unit==='hour'?amount:unit==='round'?amount/600:amount/60; };
...
export async function executeUtilitySpellCast({ base44, user, payload }) {
  ...
  if(isPassWithoutTrace||isSilence||isLongstrider)updates.conditions=structuredConditions;
