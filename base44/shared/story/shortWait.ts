...
export async function executeAuthoritativeShortWait({base44,ownerId,sessionId,characterId,requestId,actionText,receiptMetadata}){
  ...
  const receipt={request_id:requestId,version:SHORT_WAIT_VERSION,immutable:true,intent,clock:clock.clock,expired_effects:effects.expired_effects,rest:false,resources_restored:false,...(receiptMetadata?.repair_version?{repair_version:String(receiptMetadata.repair_version)}:{}),...(receiptMetadata?.proposal_hash?{proposal_hash:String(receiptMetadata.proposal_hash)}:{}),at:new Date().toISOString()};
