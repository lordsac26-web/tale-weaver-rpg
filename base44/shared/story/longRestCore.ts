...
export async function executeLongRestCore({ db, ownerId, characterId, sessionId, requestId, intent = 'long_rest_8h' }) {
  ...
  const elapsedHours = elapsedHoursForRest({ intent, startHour: getClockHour({ timeOfDay: session.time_of_day, worldState: session.world_state }) });
  ...
  const updates = { hp_current: character.hp_max, spell_slots: {}, death_saves_success: 0, death_saves_failure: 0, active_modifiers: activeModifiers, conditions, hit_dice_max: level, hit_dice_remaining: Math.min(level, (character.hit_dice_remaining ?? level) + Math.max(1, Math.floor(level / 2))), short_rest_abilities: {}, long_rest_abilities: {} };
