...
export default async function(req) {
  ...
  try {
    ...
    results.push({ name: 'genuine internal suite outputs aggregate to all_pass and one record', pass: first.all_pass === true && records.length === 1 && first.functions_run?.length === MANDATORY_NIGHTLY_SUITES.length && first.total > 0 });
    ...
    results.push({ name: 'missing mandatory suite fails closed', pass: !missing.valid && missing.missing.includes(MANDATORY_NIGHTLY_SUITES.at(-1)) });
