...
export default async function testSkeletonEnemyTurnRegression(req) {
  ...
  try {
    ...
    const run = async (label, options, rolls = [15], tacticId = 'default') => {
      const fixture = await createFixture(label, options); const before = await base44.asServiceRole.entities.CombatLog.get(fixture.combat.id); let consumed = 0;
      const response = await handleEnemyTurn({ base44, session_id: fixture.session.id, combat_id: fixture.combat.id, internal: { rollD20: () => rolls[Math.min(consumed++, rolls.length - 1)], rollDamage: () => rolls[Math.min(consumed++, rolls.length - 1)], chooseTactic: (archetype) => ({ id: tacticId, numAttacks: tacticId === 'multiattack' ? 2 : 1, attackBonus: 0, bonusDamage: 0, desc: 'deterministic', archetype }) } }); const body = await response.json();
      ...
      return { fixture, before, body, afterCombat, afterCharacter, entry, consumed, injected: { rolls, tacticId } };
    };
    ...
    const control = await run('cr_one_control', { cr: 1, numAttacks: 2 }, [15, 6, 15, 6], 'multiattack'); results.push({ name: 'CR 1 retains legitimate soldier multiattack', pass: control.entry.action === 'soldier:multiattack' && control.entry.attack_count === 2 && control.entry.damage_rolls.length === 2 && control.afterCharacter.hp_current === 24 });
