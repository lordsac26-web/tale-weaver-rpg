...
const withRandom = async (value, work) => { const original = Math.random; Math.random = () => value; try { return await work(); } finally { Math.random = original; } };
...
export default async function testSkeletonEnemyTurnRegression(req) {
  ...
  try {
    ...
    const run = async (label, options, random = 0.9) => {
      const fixture = await createFixture(label, options); const before = await base44.asServiceRole.entities.CombatLog.get(fixture.combat.id);
      const response = await withRandom(random, () => handleEnemyTurn({ base44, session_id: fixture.session.id, combat_id: fixture.combat.id })); const body = await response.json();
      ...
      return { fixture, before, body, afterCombat, afterCharacter, entry };
    };
    ...
    const oldMiss = await run('old_miss', { cr: '1/4', numAttacks: 1 }, 0); results.push({ name: 'normal miss preserves HP', pass: !oldMiss.entry.hit && oldMiss.entry.damage === 0 && oldMiss.afterCharacter.hp_current === 40 && oldMiss.afterCombat.current_turn_index === 0 });
    const oldCrit = await run('old_critical', { cr: '1/4', numAttacks: 1 }, 0.9999); results.push({ name: 'critical doubles dice only', pass: oldCrit.entry.hit && oldCrit.entry.critical && oldCrit.entry.damage === 14 && oldCrit.entry.damage_rolls[0].rolls.length === 2 && oldCrit.afterCharacter.hp_current === 26 });
    ...
    const control = await run('cr_one_control', { cr: 1, numAttacks: 2 }); results.push({ name: 'CR 1 retains legitimate soldier multiattack', pass: control.entry.action === 'soldier:multiattack' && control.entry.attack_count === 2 && control.entry.damage_rolls.length === 2 && control.afterCharacter.hp_current === 24 });
    const lowMiss = await run('low_miss', { cr: 0.125, numAttacks: 2 }, 0); results.push({ name: 'forced CR 1/8 miss preserves HP and advances exactly once', pass: !/multiattack/i.test(`${lowMiss.entry.action} ${lowMiss.entry.text}`) && !lowMiss.entry.hit && lowMiss.entry.attack_count === 1 && lowMiss.afterCharacter.hp_current === 40 && lowMiss.afterCombat.current_turn_index === 0 });
  ...
  const cleanupPassed = cleanup.every(entry => entry.deleted && entry.verified_absent); return Response.json({ cleanup, cleanup_passed: cleanupPassed, ...(output || { error: 'Skeleton regression did not produce a result' }) }, { status: output?.all_pass && cleanupPassed ? 200 : 500 });
}