...
const priorPolishRepairId = 'repair-post-rest-narration-polish-20260808';
const protectedTargetIndex = 59;
...
const diagnostics = (storyLog, targetIndex, currentText) => ({ searched_path: `story_log[${targetIndex}].text`, target_index: targetIndex, story_log_length: Array.isArray(storyLog) ? storyLog.length : 0, current_text_hash: hashValue(String(currentText || '')), occurrence_count: count(currentText, fromSentence), matching_sentences: String(currentText || '').split(/(?<=[.!?])\s+/).filter((sentence) => /boots|predator|masked|lingering|remnants|magic/i.test(sentence)).slice(0, 12) });
...
export default async function repairCraigPostRestLingeringMagic(req) {
  try {
    ...
    const priorReceipt = session.world_state?.narration_polish_repairs?.[priorPolishRepairId];
    const receiptLatestStoryIndex = Number.isInteger(priorReceipt?.latest_story_index) ? priorReceipt.latest_story_index : null;
    const targetIndex = receiptLatestStoryIndex ?? protectedTargetIndex;
    const entry = session.story_log?.[targetIndex];
    const currentText = String(entry?.text ?? '');
    const readDiagnostics = diagnostics(session.story_log, targetIndex, currentText);
    ...
    const narrativeChecks = { target_index_in_range: targetIndex >= 0 && targetIndex < (session.story_log || []).length, target_entry_exists: !!entry, rested_sentence_once: count(currentText, restedSentence) === 1, from_once: count(currentText, fromSentence) === 1, to_absent: count(currentText, toSentence) === 0 };
    ...
    const before = { character: hashValue(semantic(character)), combat: hashValue(semantic(combat)), choices: hashValue(entry.choices || []), latest_metadata: hashValue({ ...entry, text: undefined }), session_nonnarration: hashValue(sessionWithoutChangedTextAndReceipt(session, targetIndex)), narration: hashValue(currentText) };
    ...
    const after = { character: hashValue(semantic(afterCharacter)), combat: hashValue(semantic(afterCombat)), choices: hashValue(afterEntry?.choices || []), latest_metadata: hashValue({ ...afterEntry, text: undefined }), session_nonnarration: hashValue(sessionWithoutChangedTextAndReceipt(afterSession, targetIndex)), narration: hashValue(afterText) };
    const invariants = { character_hash_unchanged: before.character === after.character, combat_hash_unchanged: before.combat === after.combat, choices_hash_unchanged: before.choices === after.choices, target_entry_metadata_hash_unchanged: before.latest_metadata === after.latest_metadata, session_nonnarration_hash_unchanged: before.session_nonnarration === after.session_nonnarration, corrected_rested_sentence_once: count(afterText, restedSentence) === 1, malformed_storm_absent: !/storm of fully rested/i.test(afterText), current_fatigue_absent: !currentFatigue(afterText), mechanics_unchanged: mechanics(afterCharacter, afterSession, afterCombat).length === 0, ...persisted.checks };
    ...
    const afterDiagnostics = diagnostics(afterSession?.story_log, targetIndex, afterText);
