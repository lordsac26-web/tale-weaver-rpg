...
export default async function repairCraigPostRestLingeringMagic(req) {
  try {
    ...
    if (receipt) return Response.json({ success: true, already_processed: true, repair_id: repairId, writes: 0, receipt });
    const index = (session.story_log || []).length - 1; const latest = session.story_log?.[index]; const original = String(latest?.text || '');
    const failedInvariants = mechanics(character, session, combat);
    const narrativeChecks = { latest_entry_exists: !!latest, rested_sentence_once: count(original, restedSentence) === 1, from_once: count(original, fromSentence) === 1, to_absent: count(original, toSentence) === 0 };
    Object.entries(narrativeChecks).filter(([, pass]) => !pass).forEach(([name]) => failedInvariants.push({ name, expected: true, actual: false }));
    if (failedInvariants.length) return Response.json({ error: 'Protected repair invariants failed.', failed_invariants: failedInvariants }, { status: 409 });
    const proposed = original.replace(fromSentence, toSentence); const proposedChecks = textChecks(original, proposed);
    if (proposedChecks.failed.length) return Response.json({ error: 'Proposed repair postconditions failed.', failed_postconditions: proposedChecks.failed, replacement_count: proposedChecks.replacement_count }, { status: 409 });
    const before = { character: hashValue(semantic(character)), combat: hashValue(semantic(combat)), choices: hashValue(latest.choices || []), latest_metadata: hashValue({ ...latest, text: undefined }), session_nonnarration: hashValue(sessionWithoutChangedTextAndReceipt(session, index)), narration: hashValue(original) };
    const nextLog = session.story_log.map((entry, i) => i === index ? { ...entry, text: proposed } : entry);
    const nextWorldState = { ...(session.world_state || {}), lingering_magic_repairs: { ...(session.world_state?.lingering_magic_repairs || {}), [repairId]: { completed_at: new Date().toISOString(), before_text_hash: before.narration, after_text_hash: hashValue(proposed), latest_story_index: index } } };
    await db.entities.GameSession.update(sessionId, { story_log: nextLog, world_state: nextWorldState });
    ...
    const afterLatest = afterSession?.story_log?.[index]; const afterText = String(afterLatest?.text || ''); const persisted = textChecks(original, afterText);
    const after = { character: hashValue(semantic(afterCharacter)), combat: hashValue(semantic(afterCombat)), choices: hashValue(afterLatest?.choices || []), latest_metadata: hashValue({ ...afterLatest, text: undefined }), session_nonnarration: hashValue(sessionWithoutChangedTextAndReceipt(afterSession, index)), narration: hashValue(afterText) };
    const invariants = { character_hash_unchanged: before.character === after.character, combat_hash_unchanged: before.combat === after.combat, choices_hash_unchanged: before.choices === after.choices, latest_metadata_hash_unchanged: before.latest_metadata === after.latest_metadata, session_nonnarration_hash_unchanged: before.session_nonnarration === after.session_nonnarration, malformed_storm_absent: !/storm of fully rested/i.test(afterText), current_fatigue_absent: !currentFatigue(afterText), mechanics_unchanged: mechanics(afterCharacter, afterSession, afterCombat).length === 0, ...persisted.checks };
