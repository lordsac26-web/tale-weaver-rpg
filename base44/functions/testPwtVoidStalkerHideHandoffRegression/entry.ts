...
export default async function testPwtVoidStalkerHideHandoffRegression(req) {
  ...
  try {
    ...
    const make=async(label,{ambiguous=false,postAction=false,mismatch=false}={})=>{
      ...
      const story={timestamp:storyAt,request_id:actionId,action:'choice',player_choice:'Hide from the Void-Stalker [Skill Check: Stealth DC18 — FAILURE (rolled 15)]',text:'The Void-Stalker spots you and combat begins.',choices:[{text:'Fight'}],skill_check:bad};
      ...
      const before={hp_current:30,xp:100,inventory_hash:await hash(fresh.inventory||[]),spell_slots_hash:await hash(fresh.spell_slots||{}),abilities_hash:await hash(fresh.long_rest_abilities||{}),currency_hash:await hash({gold:5,silver:6,copper:7}),conditions_hash:await hash([pwtCondition(character.id)])};
      const receipts=ambiguous?[bad,{...bad,id:`${bad.id}:duplicate`}]:[bad];
      await db.entities.GameSession.update(session.id,{in_combat:true,combat_state:{combat_id:combat.id},story_log:[story],world_state:{...session.world_state,__skill_check_receipts:receipts,__story_handoff_receipts:[{combat_id:combat.id,action_request_id:actionId,pre_event_snapshot:before,failed_branch:{hp_delta:-3}}]}});
      ...
      return {character:await db.entities.Character.get(character.id),session:await db.entities.GameSession.get(session.id),combat:storedCombat,other:other?.id||null,canonical,contract:{characterId:character.id,sessionId:session.id,combatId:combat.id,combatCreatedAt:storedCombat.created_date,storyAt}};
    };
