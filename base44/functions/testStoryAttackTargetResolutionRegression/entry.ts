...
export default async function(req) {
  ...
  try {
    ...
    const session = await base44.entities.GameSession.create({ character_id: character.id, title: token, in_combat: false, combat_state: {}, story_log: [{ timestamp: stealthReceipt.at, request_id: stealthReceipt.request_id, player_choice: 'Move unseen toward the lead rune carver.', text: 'The living lead rune carver is alone and unaware.', skill_check: stealthReceipt, choices: [] }], world_state: { active_concentration: { spell_name: 'Pass without Trace', concentration: true, character_id: character.id, target_id: character.id, caster_id: character.id, applied_at: '2026-01-01T00:00:00.000Z', expires_at: '2026-01-01T01:00:00.000Z', expiration_rule: 'timestamp' }, __skill_check_receipts: [stealthReceipt] }, is_active: false });
