...
import { auditCampLongRestConfirmLoop } from '../../shared/repairs/campLongRestConfirmLoopAudit.ts';
...
export default async function(req) {
  try {
    ...
    if (payload?.apply) return Response.json({ error: 'This endpoint is read-only and never applies repairs.' }, { status: 405 });
    ...
    return Response.json(await auditCampLongRestConfirmLoop({ character, session }));
  } catch (error) {
    return Response.json({ error: error.message || 'Camp long-rest audit failed.' }, { status: 500 });
  }
}