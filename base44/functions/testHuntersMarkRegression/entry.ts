...
export default async function testHuntersMarkRegression(req) {
  ...
  try {
    ...
    const first = await base44.functions.invoke('combatEngine', { action: 'player_attack', session_id: replay.session.id, combat_id: replay.combat.id, character_id: replay.character.id, request_id: 'hunters-mark-replay', payload: { target_id: replay.ritualMaster.id, spell: hunterSpell } });
    const second = await base44.functions.invoke('combatEngine', { action: 'player_attack', session_id: replay.session.id, combat_id: replay.combat.id, character_id: replay.character.id, request_id: 'hunters-mark-replay', payload: { target_id: replay.ritualMaster.id, spell: hunterSpell } });
