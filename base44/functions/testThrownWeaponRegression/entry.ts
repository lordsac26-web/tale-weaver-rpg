...
export default async function testThrownWeaponRegression(req) {
  ...
  try {
    ...
    const replay = await invoke(valid, 'hit', { item_id: dagger.equipment_id, target: 'Necrotic Wolf', outcome: { committed: true, hit: true, kill: true } });
    results.push({ name: 'attack replay consumes zero', pass: replay.body?.already_processed && replay.body?.writes === 0 });
    results.push({ name: 'unrelated inventory and equipped remain unchanged', pass: await hash({ equipped: afterHit.equipped, rope: afterHit.inventory.find((item) => item.name === 'Rope') }) === untouchedHash });
    const missFixture = await make('miss', [dagger, { name: 'Rope', quantity: 1 }]); const miss = await invoke(missFixture, 'miss', { item_id: dagger.equipment_id, target: 'Wolf', outcome: { committed: true, hit: false, kill: false } }); const missAfter = await base44.asServiceRole.entities.Character.get(missFixture.character.id);
    results.push({ name: 'committed miss consumes exactly one', pass: miss.status === 200 && !(missAfter.inventory || []).some((item) => item.equipment_id === dagger.equipment_id) });
    const ambiguous = await make('ambiguous'); const ambiguousBefore = await base44.asServiceRole.entities.Character.get(ambiguous.character.id); const ambiguousResult = await invoke(ambiguous, 'ambiguous', { target: 'Wolf', outcome: { committed: true, hit: true } }); const ambiguousAfter = await base44.asServiceRole.entities.Character.get(ambiguous.character.id);
