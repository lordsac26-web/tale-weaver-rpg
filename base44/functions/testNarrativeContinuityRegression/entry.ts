...
export default async function testNarrativeContinuityRegression(req) {
  ...
  try {
    ...
    const indexedStoryLog = Array.from({ length: 60 }, (_, index) => ({ timestamp: `entry-${index}`, action: 'choice', choices: [{ text: `Choice ${index}` }], text: `Unchanged entry ${index}.` }));
    indexedStoryLog[58] = { timestamp: 'target', action: 'choice', player_choice: 'Proceed', choices: [{ text: 'Watch the door', dc: 14, risk_level: 'medium' }], text: lingeringMagicFrom };
    indexedStoryLog[59] = { timestamp: 'wolf-encounter', action: 'combat', choices: [], text: `${corrected} The wolves close in.` };
    const scanTargets = (storyLog) => storyLog.flatMap((entry, index) => { const occurrenceCountWithinText = String(entry.text || '').split(lingeringMagicFrom).length - 1; return occurrenceCountWithinText ? [{ index, occurrenceCountWithinText }] : []; });
    const matches = scanTargets(indexedStoryLog);
    const targetIndex = matches.length === 1 && matches[0].occurrenceCountWithinText === 1 ? matches[0].index : -1;
    ...
    cases.push({ name: 'global scanner resolves target story_log[58].text and leaves later index59 wolf encounter untouched', pass: targetIndex === 58 && indexedClone[58].text === lingeringMagicTo && JSON.stringify(indexedClone[59]) === JSON.stringify(indexedStoryLog[59]) });
    cases.push({ name: 'indexed repair preserves every other story log entry and target metadata byte-identically', pass: indexedClone.every((entry, index) => index === 58 || JSON.stringify(entry) === JSON.stringify(indexedStoryLog[index])) && JSON.stringify({ ...indexedClone[58], text: undefined }) === JSON.stringify({ ...indexedStoryLog[58], text: undefined }) });
