...
export default async function testStoryCommitDeployedBundleRegression(req) {
  ...
  try {
    ...
    const code = await fetchText(assets[0].href); const markers = ['story-transition-v2.4.0', 'persistence_unconfirmed', 'persisted_pair_mismatch', 'The latest authoritative scene was restored; retry your last action.'];
    const after = await hashValue(await readProtectedDndState(base44.asServiceRole)); const result = { function_version: 'test-story-commit-deployed-bundle-v1.1.0', source_expected_transition_version: 'story-transition-v2.4.0', public_asset: assets[0].pathname.split('/').pop(), public_markers: Object.fromEntries(markers.map((marker) => [marker, code.includes(marker)])), protected_unchanged: before === after, writes: 0 };
  ...
  } catch (error) { const after = await hashValue(await readProtectedDndState(base44.asServiceRole)); return Response.json({ error: error.message, function_version: 'test-story-commit-deployed-bundle-v1.1.0', source_expected_transition_version: 'story-transition-v2.4.0', protected_unchanged: before === after, writes: 0, public_all_pass: false, all_pass: false }, { status: 500 }); }
}