...
export default async function testStoryCommitDeployedBundleRegression(req) {
  ...
  try {
    ...
    const code = await fetchText(assets[0].href); const markers = ['story-transition-v2.3.0', 'persistence_unconfirmed', 'persisted_pair_mismatch', 'The latest authoritative scene was restored; retry your last action.'];
    const after = await hashValue(await readProtectedDndState(base44.asServiceRole)); const result = { function_version: 'test-story-commit-deployed-bundle-v1.0.0', asset: assets[0].pathname.split('/').pop(), markers: Object.fromEntries(markers.map((marker) => [marker, code.includes(marker)])), protected_unchanged: before === after, writes: 0 };
    result.all_pass = Object.values(result.markers).every(Boolean) && result.protected_unchanged; return Response.json(result, { status: result.all_pass ? 200 : 500 });
  } catch (error) { const after = await hashValue(await readProtectedDndState(base44.asServiceRole)); return Response.json({ error: error.message, function_version: 'test-story-commit-deployed-bundle-v1.0.0', protected_unchanged: before === after, writes: 0, all_pass: false }, { status: 500 }); }
}