...
export default async function testContentDetailRegression(req) {
  try {
    ...
    const spellNames = ['Pass without Trace', "Hunter's Mark", 'Cure Wounds', 'Fire Bolt'];
    const spells = await Promise.all(spellNames.map(async (name) => (await base44.asServiceRole.entities.Spell.filter({ name }, 'name', 1))[0] || null));
    ...
    const results = spells.map((spell, index) => ({
      name: `${spellNames[index]} has canonical popup content`,
      pass: !!spell && text(spell.description) && text(spell.school) && Number.isFinite(spell.level) && text(spell.range) && text(spell.duration) && text(spell.components),
