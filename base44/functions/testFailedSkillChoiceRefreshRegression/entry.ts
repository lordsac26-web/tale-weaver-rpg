...
export default async function testFailedSkillChoiceRefreshRegression(req) {
  try {
    ...
    try {
      ...
      const orphanScope={characterId:character.id,sessionId:orphanSession.id,receiptKey:'orphan-repair-fixture',requestId:'orphan-dash',expected:{skill:'Athletics',dc:14,raw_d20:9,modifier_total:4,final_total:13,success:false}}; const orphanDry=await failedChoiceTransitionRepairCore({db:base44.asServiceRole,mode:'dry_run',responseFormat:'guard_only',scope:orphanScope,generateChoices:async()=>({})}); const tamperedToken=`${orphanDry.body.apply_token.slice(0,-1)}${orphanDry.body.apply_token.endsWith('a')?'b':'a'}`; const tampered=await failedChoiceTransitionRepairCore({db:base44.asServiceRole,mode:'apply',applyToken:tamperedToken,scope:orphanScope,generateChoices:async()=>({})}); const orphanApply=await failedChoiceTransitionRepairCore({db:base44.asServiceRole,mode:'apply',applyToken:orphanDry.body.apply_token,scope:orphanScope,generateChoices:async()=>({})}); const orphanAfter=await base44.asServiceRole.entities.GameSession.get(orphanSession.id); const orphanReplay=await failedChoiceTransitionRepairCore({db:base44.asServiceRole,mode:'apply',applyToken:orphanDry.body.apply_token,scope:orphanScope,generateChoices:async()=>({})});
      record('compact dry run returns guarded token only', orphanDry.body?.safe_to_repair===true && orphanDry.body?.writes===0 && orphanDry.body?.proposed_choice_count===4 && !!orphanDry.body?.expected_hashes && !!orphanDry.body?.protected_before_hash && !('replacement_choices' in orphanDry.body));
      record('tampered compact token rejects with writes zero', tampered.status===409 && tampered.body?.writes===0);
      record('orphan token apply appends one fresh paired entry', orphanApply.body?.writes===1 && orphanAfter.story_log.length===2 && orphanAfter.story_log.at(-1)?.request_id==='orphan-dash' && orphanAfter.story_log.at(-1)?.skill_check?.success===false && orphanAfter.story_log.at(-1)?.choices?.length===4);
      record('orphan token replay writes zero', orphanReplay.body?.replayed && orphanReplay.body?.writes===0);
    } finally {
