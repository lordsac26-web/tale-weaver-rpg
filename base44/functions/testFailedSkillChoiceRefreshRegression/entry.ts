...
export default async function testFailedSkillChoiceRefreshRegression(req) {
  try {
    ...
    return Response.json({ function_version:'test-failed-skill-choice-refresh-v2.0.0',passed,failed:results.length-passed,total:results.length,all_pass:allPass,results,cleanup },{status:allPass?200:500});
