...
export default async function testFailedSkillChoiceRefreshRegression(req) {
  try {
    ...
    try {
      ...
      const repairScope={characterId:character.id,sessionId:staleSession.id,receiptKey:'repair-fixture'}; const dry=await failedChoiceTransitionRepairCore({db:base44.asServiceRole,mode:'dry_run',scope:repairScope,generateChoices:async()=>choices('repair')}); const applied=await failedChoiceTransitionRepairCore({db:base44.asServiceRole,mode:'apply',scope:repairScope,expectedHashes:dry.body.hashes,replacementChoices:dry.body.replacement_choices,proposalHash:dry.body.proposal_hash,generateChoices:async()=>choices('repair')}); const repairReplay=await failedChoiceTransitionRepairCore({db:base44.asServiceRole,mode:'apply',scope:repairScope,expectedHashes:dry.body.hashes,replacementChoices:dry.body.replacement_choices,proposalHash:dry.body.proposal_hash,generateChoices:async()=>choices('repair')});
      record('guarded repair preserves text and failed result', applied.body?.postconditions?.text_preserved && applied.body?.postconditions?.failed_result_preserved);
    ...
    return Response.json({ function_version:'test-failed-skill-choice-refresh-v1.0.0',passed,failed:results.length-passed,total:results.length,all_pass:allPass,results,cleanup },{status:allPass?200:500});
