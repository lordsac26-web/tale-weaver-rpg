...
export default async function testContentBackfillPhase1(req) {
  ...
  try {
    ...
    const stale = await applyPhase1Plan({ base44, proposalHash: stalePlan.proposal_hash, approvalId: `${token}:stale`, scopeIds: new Set([mismatchRecipient.id, donorA.id]) });
    results.push({ name: 'stale recipient hash fails closed with no backfill write', pass: stale.status === 409 && stale.body.writes === 0 });
    const wrong = await applyPhase1Plan({ base44, proposalHash: 'wrong-hash', approvalId: `${token}:wrong`, scopeIds });
