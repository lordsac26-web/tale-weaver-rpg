...
export default async function testLongRestRegression(req) {
  ...
  try {
    ...
    results.push({ name: 'Expired Pass without Trace cannot be narrated as currently masking movement', pass: !/your boots.{0,90}(?:masked|concealed).{0,90}(?:magic|pass without trace)|(?:magic|pass without trace).{0,90}(?:masks?|conceals?).{0,90}(?:boots|movement)/i.test('Your boots fall with the soft, practiced rhythm of a predator, each movement guided by hard-earned skill and sharpened awareness.') && /masked by the lingering remnants of your magic/i.test('Your boots fall with the soft, practiced rhythm of a predator, each movement masked by the lingering remnants of your magic.') });
