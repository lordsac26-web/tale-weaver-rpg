...
export default async function testLiveCombatStealthedAdvantageRegression(req) {
  ...
  try {
    ...
    results.push({name:'attack receipt publishes structured dice and exact modifier breakdown',pass:hitBody.log_entry?.roll_breakdown?.dice?.mode==='advantage'&&hitBody.log_entry?.roll_breakdown?.modifiers?.some(x=>x.type==='ability')&&hitBody.log_entry?.roll_breakdown?.modifiers?.some(x=>x.type==='proficiency')&&hitBody.log_entry?.roll_breakdown?.modifiers?.reduce((sum,x)=>sum+Number(x.value||0),0)===hitBody.log_entry?.roll_breakdown?.modifier_total});
