...
export default async function testApplyCraigArrowSixRepairV2(req) {
  ...
  try {
    ...
    const makeFixture = async (label, mutate = () => {}) => {
      ...
      const session = await base44.asServiceRole.entities.GameSession.create({ character_id: character.id, title: name, is_active: false });
      ...
      const combat = await base44.asServiceRole.entities.CombatLog.create({ session_id: session.id, character_id: character.id, is_active: true, result: 'ongoing', current_turn_index: 0, combatants: [player], log_entries: logs });
