...
export default async function repairCraigPostRestNarrationPolish(req) {
  try {
    ...
    if (!expectedMechanics(character, session, combat) || !actualMalformedSentence || (payload.expected_malformed_sentence !== undefined && payload.expected_malformed_sentence !== actualMalformedSentence)) return Response.json({ error: 'Expected protected mechanics or exact malformed latest narration was not present; no update applied.' }, { status: 409 });
