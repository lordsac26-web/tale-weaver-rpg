...
const malformedSentences = [`My ${malformedRemainder}`, `Your ${malformedRemainder}`];
...
export default async function repairCraigPostRestNarrationPolish(req) {
  try {
    ...
    const actualMalformedSentence = malformedSentences.find((sentence) => latest?.text?.split(sentence).length === 2);
    const failedInvariants = failedMechanics(character, session, combat);
    if (!actualMalformedSentence) failedInvariants.push({ name: 'exact_latest_malformed_sentence', expected: 'one exact My mind or Your mind malformed sentence in the latest entry', actual: normalizeSentence(latest?.text).slice(0, 300) });
    if (payload.expected_malformed_sentence !== undefined && (!actualMalformedSentence || normalizeSentence(payload.expected_malformed_sentence) !== normalizeSentence(actualMalformedSentence))) failedInvariants.push({ name: 'expected_malformed_sentence_match', expected: normalizeSentence(actualMalformedSentence), actual: normalizeSentence(payload.expected_malformed_sentence) });
    ...
    const correctedText = latest.text.replace(actualMalformedSentence, correctedSentence);
