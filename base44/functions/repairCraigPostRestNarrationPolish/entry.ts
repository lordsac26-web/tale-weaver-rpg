...
const semanticRecord = (record, omitCreated = false) => { const { updated_date, ...withoutUpdated } = record || {}; if (!omitCreated) return withoutUpdated; const { created_date, ...withoutSystemDates } = withoutUpdated; return withoutSystemDates; };
const withoutReceiptAndNarration = (session, index) => { const cleanSession = semanticRecord(session, true); return { ...cleanSession, world_state: { ...(cleanSession.world_state || {}), narration_polish_repairs: undefined }, story_log: (cleanSession.story_log || []).map((entry, entryIndex) => entryIndex === index ? { ...entry, text: undefined } : entry) }; };
const textPostconditions = (originalText, proposedText, from) => {
  const occurrenceCount = String(originalText || '').split(from).length - 1;
  const replacementCount = occurrenceCount === 1 && proposedText === String(originalText).replace(from, correctedSentence) ? 1 : 0;
  const prefix = String(originalText || '').slice(0, String(originalText || '').indexOf(from));
  const suffix = String(originalText || '').slice(String(originalText || '').indexOf(from) + from.length);
  const checks = { replacement_count_exactly_one: replacementCount === 1, malformed_sentence_absent: !String(proposedText || '').includes(from), storm_phrase_absent: !/storm of fully rested/i.test(String(proposedText || '')), corrected_sentence_once: String(proposedText || '').split(correctedSentence).length === 2, text_outside_target_unchanged: replacementCount === 1 && proposedText === `${prefix}${correctedSentence}${suffix}` };
  return { replacement_count: replacementCount, checks, failed: Object.entries(checks).filter(([, pass]) => !pass).map(([name]) => ({ name, expected: name === 'corrected_sentence_once' ? 'one exact corrected sentence' : true, actual: name === 'replacement_count_exactly_one' ? occurrenceCount : false })) };
};
...
export default async function repairCraigPostRestNarrationPolish(req) {
  try {
    ...
    const proposedText = latest.text.replace(actualSentence, correctedSentence);
    const proposedPostconditions = textPostconditions(latest.text, proposedText, actualSentence);
    if (proposedPostconditions.failed.length) return Response.json({ error: 'Narration polish postconditions failed before write.', failed_postconditions: proposedPostconditions.failed, replacement_count: proposedPostconditions.replacement_count }, { status: 409 });
    const before = { narration_text: hashValue(latest.text), story_metadata: hashValue({ ...latest, text: undefined }), choices: hashValue(latest.choices || []), character: hashValue(semanticRecord(character)), session_without_narration_and_receipt: hashValue(withoutReceiptAndNarration(session, index)), combat: hashValue(semanticRecord(combat)) };
    const nextLog = session.story_log.map((entry, entryIndex) => entryIndex === index ? { ...entry, text: proposedText } : entry);
    const receipt = { completed_at: new Date().toISOString(), latest_story_index: index, before_text_hash: before.narration_text, after_text_hash: hashValue(proposedText) };
    ...
    const after = { narration_text: hashValue(afterLatest?.text), story_metadata: hashValue({ ...afterLatest, text: undefined }), choices: hashValue(afterLatest?.choices || []), character: hashValue(semanticRecord(afterCharacter)), session_without_narration_and_receipt: hashValue(withoutReceiptAndNarration(afterSession, index)), combat: hashValue(semanticRecord(afterCombat)) };
    const persistedPostconditions = textPostconditions(latest.text, afterLatest?.text, actualSentence);
    const assertions = { character_hash_unchanged: before.character === after.character, choices_hash_unchanged: before.choices === after.choices, story_metadata_hash_unchanged: before.story_metadata === after.story_metadata, combat_hash_unchanged: before.combat === after.combat, session_nonnarration_hash_unchanged_except_receipt: before.session_without_narration_and_receipt === after.session_without_narration_and_receipt, ...persistedPostconditions.checks, protected_mechanics_intact: failedMechanics(afterCharacter, afterSession, afterCombat).length === 0 };
    const failedPostconditions = [...persistedPostconditions.failed, ...Object.entries(assertions).filter(([, pass]) => !pass).map(([name]) => ({ name, expected: true, actual: false }))];
    if (failedPostconditions.length) return Response.json({ error: 'Post-write protection assertions failed.', failed_postconditions: failedPostconditions, hashes: { before, after }, assertions, replacement_count: persistedPostconditions.replacement_count }, { status: 500 });
    return Response.json({ success: true, repair_id: repairId, writes: 1, sentences: { before: actualSentence, after: correctedSentence }, replacement_count: persistedPostconditions.replacement_count, hashes: { before, after }, assertions, expected_state: { character: 'exhaustion 0; HP 44/44; hit dice 5/5; empty spell slots; persistent Alert only; no modifiers', session: 'no active Pass without Trace concentration; current choices and story metadata preserved', combat: 'active protected combat unchanged: player turn index 0, initiative_total 18, three specified wolves alive at 15/15' } });
