...
export default async function repairCraigArrowDesync(req) {
  try {
    ...
    const matchingAttackLogs = playerAttackLogs.filter((entry) => entry?.hit === true && Number(entry?.damage) === 5 && String(entry?.target || '') === 'Corrupted Wolf');
    ...
    const ownerOverride = body?.owner_approved_verified_qa_baseline_override === true && Number(body?.verified_qa_baseline) === 8 && Number(body?.expected_successful_shots) === 1 && Number(body?.expected_current_arrows) === 7;
    const guards = {
      ...
      exact_later_arrow_shot_evidence: hasLongbow && playerAttackLogs.length === 1 && matchingAttackLogs.length === 1,
    ...
    const diagnostics = { guards, failed_guards: failedGuards, owner_approved_verified_qa_baseline_override_used: ownerOverride, before_arrow_rows: arrowRows.map(({ item, index }) => ({ index, item })), inferred_transition: '8 -> 1 shot -> 7', exact_attack_evidence: { has_longbow: hasLongbow, player_weapon_attack_logs: playerAttackLogs, matching_successful_attack: matchingAttackLogs }, hashes: { character_semantic_excluding_inventory_receipt: await hash(characterSemantic(character)), session: await hash(semantic(session)), combat: await hash(semantic(combat)), non_arrow_inventory: await hash(inventory.filter((item) => !ammoName(item))) } };
    ...
    const nextInventory = inventory.flatMap((item, index) => index === firstIndex ? [{ ...shell, name: 'Arrows', category: 'Ammunition', quantity: 7, unit: 'arrow', stack_semantics: 'individual' }] : index === 14 ? [] : [item]);
