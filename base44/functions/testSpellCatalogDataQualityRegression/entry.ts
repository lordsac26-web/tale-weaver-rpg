...
const choose = (rows) => [...rows].sort((a, b) => Number(text(b.description)) - Number(text(a.description)) || String(a.id).localeCompare(String(b.id)))[0];
...
export default async function(req) {
  try {
    ...
    const duplicateGroups = Object.values(Object.groupBy(spells, row => row.name)).filter(rows => rows.length > 1 && rows.some(row => text(row.description)) && rows.some(row => !text(row.description)));
