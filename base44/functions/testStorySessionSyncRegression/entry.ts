...
export default async function testStorySessionSyncRegression(req) {
  ...
  try {
    ...
    results.push({ name: 'owner-attributed long-rest core synchronizes character and session once', pass: first.status === 200 && afterFirstCharacter.hp_current === 24 && JSON.stringify(afterFirstCharacter.spell_slots) === '{}' && afterFirstCharacter.active_modifiers.length === 0 && afterFirstSession.world_state?.__rest_receipts?.length === 1 });
