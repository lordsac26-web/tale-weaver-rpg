...
export default async function contentBackfillPhase1(req) {
  try {
    ...
    if (mode === 'dry_run') {
      ...
      return Response.json({ ...plan, apply_requirements: { proposal_hash: plan.proposal_hash, approval_id: 'required non-empty string' }, live_apply_performed: false });
    }
    const result = await applyPhase1Plan({ base44, proposalHash: input?.proposal_hash, approvalId: input?.approval_id });
