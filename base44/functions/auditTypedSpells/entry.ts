...
Deno.serve(async (req) => {
  try {
    ...
    const story = (session?.story_log || []).map((e, i) => ({ index: i, timestamp: e.timestamp, player_choice: e.player_choice, text: e.text }))
      .filter(e => SPELL_RE.test(String(e.player_choice || '')) || SPELL_RE.test(String(e.text || ''))).slice(-30);
    ...
    for (const log of logs) {
      for (const [i, e] of (log.combat_history || []).entries()) {
        ...
        if (SPELL_RE.test(raw)) combat.push({ combat_id: log.id, updated_date: log.updated_date, index: i, entry: e });
      }
    }
