...
import { MISSED_STEALTHED_ATTACK_CONTRACT, auditRepairMissedStealthedAttackCore } from '../../shared/repairs/missedStealthedAttack.ts';
...
export default async function auditRepairMissedStealthedAttack(req) {
  try {
    ...
    const exact = body?.combat_id === MISSED_STEALTHED_ATTACK_CONTRACT.combatId && body?.character_id === MISSED_STEALTHED_ATTACK_CONTRACT.characterId && body?.session_id === MISSED_STEALTHED_ATTACK_CONTRACT.sessionId;
    ...
    const result = await auditRepairMissedStealthedAttackCore({ db: base44.asServiceRole, scope: { combatId: body.combat_id, characterId: body.character_id, sessionId: body.session_id }, mode: body.mode, requestId: body.request_id, expectedHashes: body.expected_hashes });
