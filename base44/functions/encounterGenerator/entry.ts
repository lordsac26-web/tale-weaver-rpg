...
Deno.serve(async (req) => {
  try {
    ...
    // D&D 5e DMG XP thresholds per character per level (Table: "XP Thresholds by Character Level")
    ...
    const levelThresholds = XP_THRESHOLDS_BY_LEVEL[Math.min(20, partyLevel)] || XP_THRESHOLDS_BY_LEVEL[1];
    const xpPerCharacter = levelThresholds[difficulty] || levelThresholds.medium;
