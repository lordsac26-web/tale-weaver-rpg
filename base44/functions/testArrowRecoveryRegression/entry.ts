...
export default async function testArrowRecoveryRegression(req) {
  ...
  try {
    ...
    const solasPayload = { repair_id: 'repair-craig-arrow-desync-20260808-01', character_id: 'protected-character', session_id: 'protected-session', combat_id: 'protected-combat', owner_approved_verified_qa_baseline_override: true, verified_qa_baseline: 8, expected_qualifying_attacks: 2, expected_current_arrows: 6 };
    ...
    results.push({ name: 'Solas payload shape carries exact override and two-attempt transition values', pass: solasPayload.owner_approved_verified_qa_baseline_override === true && solasPayload.verified_qa_baseline === 8 && solasPayload.expected_qualifying_attacks === 2 && solasPayload.expected_current_arrows === 6 });
