...
export default async function auditRepairLatestNarratedProjectileRecovery(req) {
  try {
    ...
    return await handleNarratedProjectileRecoveryAudit({ base44, user, rawBody });
  } catch (error) {
    return Response.json({ error: error.message || 'Narrated projectile recovery audit failed', writes: 0 }, { status: 500 });
  }
}