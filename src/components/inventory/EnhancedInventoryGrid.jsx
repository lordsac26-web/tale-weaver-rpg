...
import { Coins, Weight, Trash2, Info, Package, Sword, Shield } from 'lucide-react';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
/**
 * Enhanced InventoryGrid with drag-and-drop organization,
 * equipment slot integration, and comprehensive item management.
 */
            ...
            {(provided) => (
              ...
              >
                {items.map((item, idx) => {
                  ...
                  return (
                    <Draggable key={item.id || item.name || idx} draggableId={item.id || item.name || String(idx)} index={idx}>
                      {(provided, snapshot) => (
                        <div
                          ref={provided.innerRef}
